...
matrix = customer_data.select_dtypes(include=[np.number]).corr() # YOUR CODE HERE
...
matrix_max = round(np.max(matrix.values[(matrix.values < 1) & (matrix.values > -1)]), 2) # 1.4 of Gradescope tests
...
kmeans = KMeans(n_clusters=chosen_cluster, init='k-means++', n_init=10, random_state=42) # write a KMeans instance with the following prescribed parameters - n_clusters = the chosen cluster value as reported above, init = 'k-means++', n_init = 10 and random_state = 42.fit(customer_data)
